...
{
    class HttpRequestParser : public Request
    {
    public:
        HttpRequestParser()
    ...
    private:
        ...
        {
            return stricmp(item.name.c_str(), "Connection") == 0;
        }
        ...
        ParseResult consume(Request& req, const char* begin, const char* end)
        {
            while (begin != end)
            {
                ...
                case HeaderValue:
                    if (input == '\r')
                    {
                        if (req.method == "POST" || req.method == "PUT")
                        {
                            ...
                            if (stricmp(h.name.c_str(), "Content-Length") == 0)
                            {
                                contentSize = atoi(h.value.c_str());
                                req.content.reserve(contentSize);
                            }
                            else if (stricmp(h.name.c_str(), "Transfer-Encoding") == 0)
                            {
                                if (stricmp(h.value.c_str(), "chunked") == 0)
                                    chunked = true;
                            }
                        }
                        state = ExpectingNewline_2;
                ...
                case ExpectingNewline_3: {
                    ...
                    {
                        if (stricmp(it->value.c_str(), "Keep-Alive") == 0)
                        {
                            req.keepAlive = true;
                        }
                        else  // == Close
