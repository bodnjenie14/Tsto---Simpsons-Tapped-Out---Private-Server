...
#undef EXCEPTION_CASE
		...
		std::string get_callstack_summary(void* exception_addr, int trace_depth = 32)
		{
			...
			for (size_t i = exception_start_index; i < count; i++)
			{
				...
				if (from.get_name() == "server.exe"s) rva += 0x140000000;
		...
		std::string generate_crash_info(const LPEXCEPTION_POINTERS exceptioninfo)
		{
			...
			const auto build_info = "Bodnjenie TsTO Emulator"s;
			...
			line(utils::string::va("Thread ID: %d(%s)", GetCurrentThreadId(), is_main_thread() ? "Main Thread" : "Auxiliary Threads"));
		...
		{
			const std::string crash_name = utils::string::va("minidumps/server-crash-%s.zip",
			                                                 get_timestamp().data());

			utils::compression::zip::archive zip_file{};
			zip_file.add("crash.dmp", exception::create_minidump(exceptioninfo));
			zip_file.add("info.txt", generate_crash_info(exceptioninfo));
			zip_file.write(crash_name, "Server Crash Dump");
		}
		...
		LONG WINAPI exception_filter(const LPEXCEPTION_POINTERS exceptioninfo)
		{
			...
			logger::write(logger::LOG_LEVEL_CRITICAL, logger::LOG_LABEL_BLACKBOX, "Fatal error (0x%08X) at 0x%p (0x%p). A minidump has been written.", exception_data.code, exception_data.address, reinterpret_cast<uint64_t>(exception_data.address));
			...
			MessageBoxA(nullptr, error_str.data(), "Bodnjenie TsTO Emulator Error", MB_ICONERROR);
